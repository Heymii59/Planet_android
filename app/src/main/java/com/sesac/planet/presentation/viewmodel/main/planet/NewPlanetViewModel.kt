...
class NewPlanetViewModel(private val createNewPlanetUseCase: CreateNewPlanetUseCase) : ViewModel() {
    private val _newPlanetData = MutableLiveData<Response<CreateNewPlanetResponse>>()
    val newPlanetData: LiveData<Response<CreateNewPlanetResponse>> get() = _newPlanetData

    init {
        PostPlanetRepository.createNewPlanetService = PlanetApplication.getInstance().create(PostPlanetAPI::class.java)
    }
